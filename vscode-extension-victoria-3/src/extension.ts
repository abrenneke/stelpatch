...
/**
 * Check if a file path contains a 'common' directory, indicating it's a Victoria 3 config file
 */
function isVictoria3File(filePath: string): boolean {
...
function setLanguageForDocument(document: TextDocument) {
	if (document.languageId === 'plaintext' && isVictoria3File(document.uri.fsPath)) {
		// TODO causes recursion error or infinite loop or something :(
		// log(`🎯 Auto-detecting Victoria 3 file: ${document.uri.fsPath}`);
		// languages.setTextDocumentLanguage(document, 'victoria3');
	}
}
...
export function activate(context: ExtensionContext) {
	...
	if (fs.existsSync(serverExecutable)) {
		...
		const serverArgs = ['run', '--release', '--bin', 'cw_lsp', '--no-default-features', '--features', 'victoria_3'];
	...
	const clientOptions: LanguageClientOptions = {
		// Register the server for Victoria 3 documents
		documentSelector: [
			{ scheme: 'file', language: 'victoria3' },
