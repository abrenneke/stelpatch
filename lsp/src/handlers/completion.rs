...
fn repair_syntax_for_completion(content: &str, offset: usize) -> Option<(String, usize)> {
    ...
    if let Some(equals_pos) = line_up_to_cursor.rfind('=') {
        ...
        // Simple repair: add placeholder for empty values
        if after_equals_trimmed.is_empty() {
            ...
            for (i, line) in lines.iter().enumerate() {
                if i < current_line_index {
                    ...
                    repaired_content.push_str(line_up_to_cursor);
            ...
            let adjusted_offset = offset + 2; // +2 for space and opening quote
            return Some((repaired_content, adjusted_offset));
        }
    }
...
pub async fn completion(
    ...
    match cached_document.borrow_ast() {
        ...
        Err(_) => {
            // Try to repair the syntax by inserting placeholder values
