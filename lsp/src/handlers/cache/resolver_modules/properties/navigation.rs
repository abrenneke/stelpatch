...
use crate::handlers::{
    cache::{
        ...
        resolver_modules::properties::{
            handlers::{
                handle_pattern_property, handle_regular_property, handle_subtype_pattern_property,
                handle_subtype_property,
            },
            links::is_link_property,
            scope_changes::apply_alias_scope_changes,
            subtypes::{get_subtype_pattern_property, get_subtype_property},
        },
    },
    scope::ScopeError,
...
pub fn navigate_to_block_property(
    ...
    for subtype_name in scoped_type.subtypes() {
        ...
        if let Some(subtype_pattern_property) = get_subtype_pattern_property(
            pattern_matcher.clone(),
            block,
            subtype_name,
            property_name,
        ) {
            let result = handle_subtype_pattern_property(
                cwt_analyzer.clone(),
    ...
    if let Some(pattern_property) = pattern_matcher.key_matches_pattern(property_name, block) {
...
pub fn navigate_to_alias_property(
    ...
    let (resolved_cwt_type, alias_def, scripted_effect_name) =
        reference_resolver.resolve_alias_match_left(key, property_name);
