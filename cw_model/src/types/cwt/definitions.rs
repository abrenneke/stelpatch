...
pub struct TypeDefinition {
    ...
    pub localisation: HashMap<Spur, LocalisationRequirement>,
...
impl LocalisationRequirement {
    ...
    pub fn required(pattern: Spur) -> Self {
        Self {
            pattern,
            required: true,
            primary: false,
    ...
    pub fn primary(pattern: Spur) -> Self {
        Self {
            pattern,
            required: false,
            primary: true,
            ...
            for (subtype_name, properties) in block_type.subtype_properties.iter() {
                if let Some(subtype) = block_type.subtypes.get_mut(subtype_name) {
                    ...
                    for (prop_key, prop_value) in properties {
                        subtype
                            .allowed_properties
                            .insert(*prop_key, prop_value.clone());
                    }
                }
            }
...
impl EnumDefinition {
    ...
    pub fn new_simple(values: HashSet<Spur>) -> Self {
        Self {
            values,
            complex: None,
        }
    ...
    pub fn get_all_values(&self) -> &HashSet<Spur> {
        &self.values
    }
}
...
impl AliasDefinition {
    ...
    pub fn new(category: Spur, name: Spur, rules: Arc<CwtType>) -> Self {
        Self {
            category,
            name,
            to: rules,
...
impl LinkDefinition {
    ...
    pub fn new(name: Spur, input_scopes: Vec<Spur>, output_scope: Spur) -> Self {
        Self {
            name,
            desc: None,
            from_data: false,
    ...
    pub fn with_desc(mut self, desc: Spur) -> Self {
        self.desc = Some(desc);
        self
    }
    ...
    /// Set data source
    pub fn with_data_source(mut self, data_source: Spur) -> Self {
        self.data_source = Some(data_source);
        self
    }

    /// Set prefix
    pub fn with_prefix(mut self, prefix: Spur) -> Self {
        self.prefix = Some(prefix);
        self
    }

    /// Check if this link can be used from a specific scope
    pub fn can_be_used_from(
        &self,
        scope: Spur,
        ...
        if let Some(scope_name) = analyzer.resolve_scope_name(scope) {
            return self.input_scopes.iter().any(|s| {
                interner.resolve(&s) == "all" || analyzer.resolve_scope_name(*s) == Some(scope_name)
            });
        }
...
impl ScopeDefinition {
    ...
    pub fn new(name: Spur, aliases: Vec<Spur>) -> Self {
        Self { name, aliases }
    }

    /// Check if this scope has a specific alias
    pub fn has_alias(&self, alias: Spur) -> bool {
        self.aliases.contains(&alias)
    }

    /// Get all identifiers for this scope (name + aliases)
    pub fn all_identifiers(&self) -> Vec<Spur> {
...
impl ScopeGroupDefinition {
    ...
    pub fn new(name: Spur, members: Vec<Spur>) -> Self {
        Self { name, members }
    }

    /// Check if this group contains a specific scope
    pub fn contains_scope(&self, scope: Spur) -> bool {
